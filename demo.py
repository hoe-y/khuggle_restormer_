...
def calculate_psnr(img1, img2):
    img1 = img1.astype(np.float64).astype(np.uint8)
    img2 = img2.astype(np.float64).astype(np.uint8)
...
with torch.no_grad():
    ...
    for file_ in tqdm(files):
        ...
        if task == 'Gaussian_Gray_Denoising':
            ...
            #print(str(file_))
            img = load_img(file_)
            #file_gt = file_[:35]+'val_gt'+file_[38:]#56]+'GT'+file_[58:63]+'0001.jpg'
            #print(str(file_gt))
            #gt = load_img(file_gt)
        ...
        if task == 'Gaussian_Gray_Denoising':
            ...
            #print(calculate_psnr(restored, gt))
